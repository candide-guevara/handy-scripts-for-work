...
TMP_HELP_FILE="$HOME/Temp/help.autogenerated.tmp"
...
## *USAGE : custom_ssh [STANDARD SSH OPTIONS]
## Simple wrapper around ssh to factor out common options.
custom_ssh() {
  ssh_key_option=""
  if [[ ! -z $MY_SSH_KEY ]]; then
    ssh_key_option=( -i $HOME/.ssh/$MY_SSH_KEY )
  fi
  command=( ssh -o ServerAliveCountMax=10 -o ServerAliveInterval=60 ${ssh_key_option[@]} $@ )
  run_cmd ${command[@]}
}
